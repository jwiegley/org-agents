...
> **Superseded in part, 2026-08-19 (Epic 1 of the dynamic-properties work).**
> Four statements below no longer describe the code, and the changes are in
> README.md §"The query language" and §"Commands and options":
>
...
>    why every `org-agents` option now carries `:risky t`.
> 3. "the gate runs before normalization, on the raw expanded sexp" is still
>    true and is no longer the whole answer: running first does not stop a
>    normalizer that runs afterwards. `org-agents-refused-heads` refuses such
>    a head outright, ahead of the safe list and of
