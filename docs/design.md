...
Expansion is a pure function from the read sexp to `(:full QUERY :skeleton SEXP-STRING-OR-NIL)`. `:full` is standard org-ql; `:skeleton` is the CLI-dialect string for the prefilter, produced by `prin1` from the *pre-normalization* sexp (org-ql normalizers create `#s(ts …)` structs the CLI cannot read; `read` has already turned `+7` into `7`). The expander and the splitter recurse over the same form set: `and`/`or`/`not`/`when`/`unless` plus the nested-query arguments of `parent`, `ancestors`, `children`, `descendants` — `$refs` are expanded inside nested queries too.

**As implemented, the splitter is narrower than the sentence above suggests, deliberately.** The *expander* recurses over that whole form set; the *splitter* walks only the top-level `and` conjuncts (or the whole query when it is a single pushable predicate). It does not traverse `or` or `not`, and a nested query pushes nothing at all. So "a nested query's push-down class is the class of its weakest member" is not implemented: the class is simply "not pushed". This is sound — every unpushed conjunct stays residual and only widens the candidate set — and is the conservative reading. Traversing `or`/`not` and computing a weakest-member class for nested queries is a v2 item, not a defect in v1.
...
| `(scheduled …)` `(deadline …)` `(closed …)` | `:from/:to/:on` only, each value resolved to an absolute **local** `"YYYY-MM-DD"` | planning stamps stored for exactly these; MJD-consistent. `today` and integer offsets must be resolved on the Elisp side: the CLI resolves a relative value against `utctDay <$> getCurrentTime` (`bin/DB/Exec.hs`) while org-ql uses `(ts-now)`, so a relative value pushed verbatim names a different day to each engine and under-matches for part of every day. Resolved with `ts-adjust`, which is what org-ql itself uses, so the day arithmetic cannot drift (seconds arithmetic differs across a DST change). org-ql treats an absolute string exactly as it treats `today` — `:from`/`:on` floored, `:to` ceilinged — so the substitution is exact |
